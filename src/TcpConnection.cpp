...
TcpConnection::TcpConnection(EventLoop *loop, const std::string &nameArg, int sockfd, const InetAddress &localAddr, const InetAddress &peerAddr)
    : loop_(Check_notnull(loop)), name_(nameArg), state_(StateE::kConnecting), reading_(true), socket_(new Socket(sockfd)), channel_(new Channel(loop, sockfd)), localAddr_(localAddr), peerAddr_(peerAddr), highWaterMark_(64 * 1024 * 1024)
{
...
void TcpConnection::sendInLoop(const void *data, size_t len)
{
    ...
    // poller发现tcp的发送缓冲区有空间,会通知相应的sock->channel.调用handleWrite回调
...
void TcpConnection::handleWrite()
{
    ...
    {
        LOG_INFO("TcpConnection fd = %d is downm no more writing \n", channel_->fd());
    }
}
