...
{
    LOG_DEBUG("EventLoop created " + std::to_string((int)this) + " in thread: " + std::to_string(threadId_) + '\n');
...
void EventLoop::loop()
{
    ...
    while (!quit_)
    {
        ...
        /*
         * IO线程 mainReactor accept接收新用户的连接,返回一个与新用户通信的fd,channel打包
         * subloop
         * mainLoop 事先注册一个回调cb(需要subloop来执行)   通过eventfd唤醒subloop后执行下面的方法,执行之前mainloop注册的cb操作
         */
        doPendingFunctors();
    }
...
// 退出事件循环
// 1.loop在自己的线程中调用quit
// 2.在其他的subloop(worker)中调用了mainloop(IO)的quit
void EventLoop::quit()
{
    quit_ = true;
...
void EventLoop::doPendingFunctors() // 执行回调
{
    ...
    {
        std::unique_lock<std::mutex> lock(mutex_);
        functors.swap(pendintFunctors_); // 交换之后pendintFunctors_置为空
    }
